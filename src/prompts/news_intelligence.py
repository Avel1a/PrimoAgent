...
class NewsFeatures(BaseModel):
    news_relevance: int = Field(...)
    sentiment: int = Field(...)
    price_impact_potential: int = Field(...)
    trend_direction: int = Field(...)
    earnings_impact: int = Field(...)
    investor_confidence: int = Field(...)
    risk_profile_change: int = Field(...)


def get_news_output_parser() -> PydanticOutputParser:
    return PydanticOutputParser(pydantic_object=NewsFeatures)
