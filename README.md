...
投资组合管理 Agent
      ↓
风险管理 Agent  ← 自适应 Regime 检测
      │            ├─ 滚动回撤（60 天窗口）
      │            ├─ 趋势强度（SMA 斜率 + MACD + ADX）
      │            └─ 波动率状态（ATR 比率）
      ↓            ┌─ Bear（收紧）| Neutral（默认）| Bull（放宽）
CSV 输出 + 回测引擎
```
...
| **风险管理** | VaR 仓位上限、最大回撤熔断、现金储备、集中度限制 + **自适应 Regime 检测** | 历史价格数据 |
...
- **自适应风控**：三信号 regime 检测（滚动回撤 + 趋势强度 + 波动率状态），动态切换 Bear/Neutral/Bull 三档参数，牛市放宽、熊市收紧
...
src/
  ...
  tools/
    ...
    daily_csv_tool.py            # 日频分析 CSV 持久化（含 regime 列）
  workflows/
...
> TSLA 单股，2026-04-08 ~ 2026-05-08（23 个交易日），初始资金 $100,000

| 策略 | 累计收益 | 年化波动 | 最大回撤 | Sharpe | 交易次数 |
|------|---------|---------|---------|--------|---------|
| PrimoAgent | +3.83% | 14.10% | 2.84% | 2.98 | 9 |
| Buy & Hold | +18.82% | 35.57% | 6.66% | 5.69 | 1 |

> TSLA 在此期间从 $343 反弹至 $411（+20%）。自适应风控通过 60 天滚动回撤替代全局峰值回撤，在反弹中逐步放宽限制（Neutral → Bull），相比改造前（+0.46% / 4 笔交易）收益提升 8 倍、交易次数翻倍，同时最大回撤控制在 2.84%。
...
**自适应 Regime 检测 (`risk.regime`)：**

三信号加权（趋势 0.4 + 滚动回撤 0.3 + 波动率 0.3）输出 regime score ∈ [-1, 1]：

| 状态 | 阈值 | 回撤限制 | 仓位上限 | VaR 乘数 | 现金储备 | HOLD 覆盖策略 |
|------|------|---------|---------|---------|---------|-------------|
| **Bear** | < -0.3 | 12% | 10% | 0.5x | 20% | BUY/SELL 全拦截 |
| **Neutral** | -0.3 ~ 0.3 | 20% | 25% | 1.0x | 10% | 仅回撤超限时 HOLD |
| **Bull** | > 0.3 | 30% | 30% | 1.5x | 5% | 仅回撤超限且趋势向下时 HOLD |
