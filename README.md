...
- Collects real-time market data via Tiingo (primary), Alpha Vantage (fallback), and Finnhub APIs
- Retrieves company profiles, financial metrics, and recent news
- Handles weekend/holiday news periods automatically
- Implements intelligent fallback: Tiingo → Alpha Vantage if unavailable
...
Copy `.env` file from the project root and fill in your API keys:

```bash
# Copy the existing .env file or create a new one with these keys
```

Required API keys in `.env`:
```bash
# LLM and General APIs
OPENAI_API_KEY=your_openai_key_here
FIRECRAWL_API_KEY=your_firecrawl_key_here
PERPLEXITY_API_KEY=your_perplexity_key_here

# Financial Data APIs (Tiingo is the preferred data source)
TIINGO_API_KEY=your_tiingo_key_here
ALPHA_VANTAGE_API_KEY=your_alpha_vantage_key_here
FINNHUB_API_KEY=your_finnhub_key_here
```

**Data Source Priority:** The system now uses Tiingo as the primary data source for market data and company information. If Tiingo API is unavailable or returns errors, the system automatically falls back to Alpha Vantage as a secondary data source. Finnhub is used for news and financial data.
